...
\begin{abstract}
Este documento presenta el desarrollo de un sistema de instrumentación virtual para la monitorización, calibración y control automático de nivel de un depósito de agua. El sistema integra una interfaz HMI desarrollada en Python con Dash, un Arduino UNO para la adquisición y actuación, y una Red Pitaya para el procesamiento del sensor capacitivo RC. El trabajo representa la culminación de los entregables anteriores, evolucionando hacia un instrumento virtual completo con capacidades de calibración automática y control en lazo cerrado.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Calibración interactiva}: Implementar un proceso automatizado para establecer la relación entre la señal del sensor RC y el nivel real mediante interpolación.
...
La columna izquierda contiene los controles de actuación, presentados como botones claramente etiquetados que permiten al operador interactuar directamente con las bombas del sistema. Cada botón tiene un color distintivo que facilita su identificación rápida: verde para el llenado, naranja para el vaciado, y rojo para la parada de emergencia. La columna derecha presenta la información de monitorización, mostrando el estado actual del sistema mediante indicadores visuales y valores numéricos. Controles disponibles:
\begin{itemize}
...
@app.callback(
    [Output('intervalo-lectura', 'disabled'),
     Output('estado-sistema', 'children')],
    [Input('btn-llenar', 'n_clicks'),
     Input('btn-vaciar', 'n_clicks'),
     Input('btn-parar', 'n_clicks')]
)
def controlar_bombas(n_llenar, n_vaciar, n_parar):
    ctx = dash.callback_context
    if not ctx.triggered:
        return True, "Sistema detenido"
    
    boton_id = ctx.triggered[0]['prop_id'].split('.')[0]
    
    if boton_id == 'btn-llenar':
        comando_llenar(arduino)
        return False, "Llenando..."
    elif boton_id == 'btn-vaciar':
        comando_vaciar(arduino)
        return False, "Vaciando..."
    else:  # btn-parar
        comando_parar(arduino)
        return True, "Sistema detenido"
\end{lstlisting}
...
Una vez adquiridos los puntos, se aplica un algoritmo de interpolación que genera una función continua capaz de estimar el peso correspondiente a cualquier valor de la señal RC. Los pasos fundamentales del proceso son:
...
\begin{enumerate}
    ...
    \item Generación de función de interpolación continua
\end{enumerate}
...
Botón Ajustar & Genera modelo de interpolación tras adquisición \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=Python, caption=Lógica de control durante calibración]
def proceso_calibracion(peso_actual, objetivo, tolerancia=5):
    diferencia = objetivo - peso_actual
    
    if abs(diferencia) <= tolerancia:
        comando_parar(arduino)
        return "MEDIDA_TOMADA"
    elif diferencia > 0:
        comando_llenar(arduino)
        return "LLENANDO"
    else:
...
Una vez completada la adquisición de los puntos de calibración, el usuario puede generar el modelo matemático que permitirá convertir las lecturas del sensor RC en valores de peso equivalentes. Este modelo es una función de interpolación que, dado un valor de integral RC, devuelve el peso estimado correspondiente.

La biblioteca SciPy proporciona la función \texttt{interp1d} que implementa varios métodos de interpolación. Para este sistema se ha elegido la interpolación lineal, que conecta los puntos de calibración mediante segmentos rectos. Aunque existen métodos más sofisticados como los splines cúbicos, la interpolación lineal ofrece ventajas importantes: es computacionalmente eficiente, no produce oscilaciones espurias entre los puntos de datos, y proporciona resultados predecibles incluso cuando los datos contienen cierto nivel de ruido.

El código que implementa la generación del modelo utiliza \texttt{scipy.interpolate.interp1d}:

\begin{lstlisting}[language=Python, caption=Generación de función de interpolación]
from scipy.interpolate import interp1d
import numpy as np

def generar_modelo_calibracion(datos_calibracion):
    integrales = np.array([d['integral'] for d in datos_calibracion])
    pesos = np.array([d['peso'] for d in datos_calibracion])
    
    # Ordenar por integral para interpolacion correcta
    indices = np.argsort(integrales)
    integrales = integrales[indices]
    pesos = pesos[indices]
    
    # Crear funcion de interpolacion lineal
    funcion_calibracion = interp1d(
        integrales, pesos,
        kind='linear',
        fill_value='extrapolate'
    )
    
    return funcion_calibracion
\end{lstlisting}

La interpolación lineal se elige por su eficiencia computacional, ausencia de oscilaciones espurias, y resultados predecibles incluso con datos ruidosos.
...
\midrule
Precisión típica & $\pm$2g & $\pm$5g (tras calibración) \\
...
\begin{itemize}
    \item \textbf{Campo consigna}: Valor objetivo en gramos (rango: 0-800g)
...
\begin{lstlisting}[language=Python, caption=Actualización de gráfica de control]
@app.callback(
    Output('grafica-control', 'figure'),
    [Input('intervalo-control', 'n_intervals')],
    [State('historial-control', 'data'),
     State('consigna-actual', 'data')]
)
def actualizar_grafica(n, historial, consigna):
    fig = go.Figure()
    
    # Nivel actual
    fig.add_trace(go.Scatter(
        x=historial['tiempo'], y=historial['nivel'],
        mode='lines', name='Nivel actual'
    ))
    
    # Consigna y banda de histeresis
    fig.add_hline(y=consigna, line_dash="dash", 
                  annotation_text="Consigna")
    fig.add_hrect(y0=consigna-10, y1=consigna+10,
...
// SOLUCION: Usar palabra en español
...
\midrule
\texttt{InterfazWeb.py} & $\sim$1180 & Aplicación Dash principal (layout + callbacks) \\
\texttt{ComWeb.py} & $\sim$80 & Funciones comunicación SCPI con Arduino \\
\texttt{redpitaya\_scpi.py} & $\sim$50 & Clase comunicación TCP/IP Red Pitaya \\
\texttt{PruebaSCPI.ino} & $\sim$200 & Firmware Arduino (parser + control) \\
\texttt{assets/styles.css} & $\sim$100 & Estilos CSS de la interfaz \\
\bottomrule
\end{tabular}
\end{table}
...
app.layout = html.Div([
    dcc.Tabs([
        ...
        dcc.Tab(label='Calibracion RC', children=[...]),
        dcc.Tab(label='Control Automatico', children=[...]),
    ]),
    
    # Componentes de almacenamiento
    dcc.Store(id='datos-calibracion', data=[]),
    dcc.Store(id='historial-control', data={'tiempo': [], 'nivel': []}),
    dcc.Store(id='funcion-calibracion', data=None),
    
    # Intervalos de actualizacion
    dcc.Interval(id='intervalo-lectura', interval=2000, disabled=True),
    dcc.Interval(id='intervalo-calibracion', interval=1000, disabled=True),
    dcc.Interval(id='intervalo-control', interval=2000, disabled=True),
])

# Callbacks para cada funcionalidad...
...
Los resultados de las pruebas de calibración con 10 puntos fueron:
...
\midrule
Tiempo de proceso & $\sim$8 minutos \\
Coeficiente $R^2$ & $>$ 0.99 \\
Error típico (RMSE) & $<$ 5g \\
...
\begin{itemize}
    \item \textbf{Tiempo hasta consigna}: $\sim$30 segundos
    \item \textbf{Respuesta a perturbaciones}: $\sim$2 segundos (limitado por intervalo de muestreo)
...
\begin{table}[H]
\centering
\caption{Dependencias de software}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Paquete} & \textbf{Versión} & \textbf{Función} \\
\midrule
Python & $\geq$ 3.8 & Entorno de ejecución \\
Dash & $\geq$ 2.0 & Framework interfaz web \\
Plotly & $\geq$ 5.0 & Gráficas interactivas \\
NumPy & $\geq$ 1.20 & Procesamiento numérico \\
SciPy & $\geq$ 1.7 & Funciones de interpolación \\
PySerial & $\geq$ 3.5 & Comunicación serie \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item Arduino NANO con firmware cargado
    \item Báscula HX711 conectada y calibrada
    \item Red Pitaya accesible en red (\texttt{rp-f082af.local})
    \item Sistema de bombas con driver conectado a pines Arduino
\end{itemize}

\subsection{Ejecución}

\begin{lstlisting}[language=bash, caption=Inicio de la aplicación]
